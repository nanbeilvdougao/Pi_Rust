...
fn complete_chat(
    ...
    complete_chat_with_headers(info, endpoint, &[("authorization", auth.as_str())], request)
}

/// Same wire format as `complete_chat`, but the caller supplies the full
/// header set. Azure (`api-key`), Copilot (custom GH headers), Cloudflare,
/// OpenAI Responses, etc. all reuse this path.
pub fn complete_chat_with_headers(
    info: &ProviderInfo,
    endpoint: &str,
    headers: &[(&str, &str)],
    request: ProviderRequest,
    ...
    let response = post_json(&agent, endpoint, &body, headers)?;
...
pub fn stream_chat_with_headers(
    info: &ProviderInfo,
    endpoint: &str,
    headers: &[(&str, &str)],
    request: ProviderRequest,
    ...
    post_sse_lines(&agent, endpoint, &body, headers, |line| {
        if sink.cancelled() {
...
pub fn build_request_body(request: &ProviderRequest, stream: bool) -> Value {
...
pub fn openrouter_info() -> ProviderInfo {
    ProviderInfo {
        ...
        local_first: false,
        requires_api_key_env: Some("OPENROUTER_API_KEY".to_string()),
    }
}

pub fn mistral_info() -> ProviderInfo {
    ProviderInfo {
        ...
        local_first: false,
        requires_api_key_env: Some("MISTRAL_API_KEY".to_string()),
    }
}
